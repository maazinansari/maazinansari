...
g = function(x, n) finney_series(x, n)

# From Ginos 2009
g_approx = function(t, n) { 
    result = exp(t) * (
        1 - t * (t + 1) / n +
...
finney_estimate_1 = function(data) {
    x = log(data)
    n = length(data)
...
finney_estimate_2 = function(data) {
    x = log(data)
    n = length(data)
    
    S_2 = var(x) # n / (n-1) * sigma_hat^2
    m_hat = exp(mean(x)) * g(S_2/2, n)
    v_hat = exp(2 * mean(x)) * (g(2 * S_2, n) - g((n-2) * S_2 / (n-1), n))
