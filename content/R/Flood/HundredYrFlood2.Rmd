...
```{r get-prev-data, output = 'hide', echo = FALSE, warning = FALSE, message = FALSE}
...
Now, let's count how many 100-year floods occured in the 1000 37-year periods. In other words, for each column (simulation), how many rows (years) exceed `r v_mle`? Then we'll make a table to see how frequent the counts are.
...
df_binom = rbind(b_table,
                 dbinom(0:max(bFloods), size = 37, prob = 0.01),
                 sim_table)
df_pois = rbind(p_table,
...
---------------------------|-----------------------------------------------------
Expected Binomial Counts   | `r dbinom(x = 1, size = 37, prob = 0.01)`
Simulated Binomial Counts  | `r b_table[["1"]]`
Expected Poisson  Counts   | `r dpois(x = 1, lambda = 37*0.01)`
Simulated Poisson Counts   | `r p_table[["1"]]`
Counts of Simulated Floods | `r sim_table[["1"]]`
